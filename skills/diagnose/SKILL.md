...
10. **HITL bash script.** Last resort. If a human must click, drive _them_ with a script that prompts step-by-step so the loop is still structured.
...
**Then ask: what would have prevented this bug?** If the answer involves architectural change (no good test seam, tangled callers, hidden coupling) hand that insight over to the user.
