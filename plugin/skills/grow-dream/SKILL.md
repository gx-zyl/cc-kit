---
name: grow-dream
description: 回顾本次对话，分析对 skill / rule / agent / hook / CLAUDE.md / AGENTS.md / memory 是否有改进空间，输出改进建议并可选择直接产出改进文件。识别有规律或频繁提醒的提示并建议写入 memory。之后调用 grill-me-2 对改进候选进行追问验收。用户说"改进"、"沉淀"、"提炼规则"、"这个经历能沉淀什么"、"有没有可以提炼成规则的模式"、"造梦"、"总结对话"、"提炼经验"、"固化模式"、"make dream"时触发。
---
...
:```ascii
  ...
  ┌────────────┐   ┌────────────┐   ┌────────────┐   ┌────────────┐   ┌────────────┐   ┌────────────┐   ┌────────────┐   ┌──────────────┐
  │  ① 确定    │   │  ② 浏览    │   │  ③ 项目    │   │  ④ 交叉    │   │  ⑤ 分类    │   │  ⑥ 结构化   │   │  ⑦ 可选    │   │  ⑧ grill-me-2│
...
9. **grill-me-2 追问验收** — 调用 `grill-me-2` skill 对改进候选进行交叉拷打（见下节）

## 第⑧步: grill-me-2 追问验收

产出改进建议后，**自动调用 `grill-me-2` skill（验收模式）** 对每个候选进行追问，防止产出 shelfware（食之无味弃之可惜的固化产物）。
...
grill-me-2 的验收结论**追加**到 grow-dream 输出末尾：
...
- [ ] **grill-me-2 验收**：每个改进候选已过 grill-me-2 追问，输出明确判定（采纳/放弃/需补充）
