...
> 管道图 ⑧ 阶段 ↔ 执行步骤映射：①=0 ②=1 ③=2 ④=3 ⑤=4 ⑥=5-6 ⑦=7-8 ⑧=9
...
9. **⑧ grill-dream 追问验收** — 调用 `grill-dream` skill（验收模式）对每个改进候选进行交叉拷打。

   **验收规则**：
   | 候选类型 | 验收问题 | 通过标准 |
   |---------|---------|---------|
   | 高频模式 | 频次够不够？跨会话通用？ | 频次 ≥2，跨 ≥2 个会话 |
   | memory | 满足双门槛？已有覆盖？ | 频次≥2 + 跨会话 |
   | skill | 触发明确？无重叠？ | 独立场景，无已有覆盖 |
   | command | 步骤固定 ≤7？ | 纯确定性操作 |
   | rule/agent/hook | 适用面够广？ | 通用且必要 |

   验收结论以表格追加到输出末尾（见输出格式 `## grill-dream 验收`）。
