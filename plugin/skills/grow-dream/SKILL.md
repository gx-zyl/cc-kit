...
:```ascii
  ...
  └────────────┘   └────────────┘   └────────────┘   └────────────┘   └────────────┘   └────────────┘   └────────────┘
       ...
       │ git log         │ 隐性模式       │ rules/          │ AI反馈一致性   │ rule (通用)     │               │
       └─ 修改历史       └─ 频次统计      └─ agents/        └─              └─ agent/hook/doc └─              └─
:```
...
:```ascii
              ...
              │                                                                             │
              │  ┌─────────────────┐  ┌─────────────────┐  ┌──────────────────────────┐     │
              │  │  skill 定义      │  │  rule 约束       │  │  agent 角色              │     │
              │  │ plugin/skills/  │  │ .claude/rules/  │  │ .claude/agents/         │     │
              │  └────────┬────────┘  └────────┬────────┘  └───────────┬──────────────┘     │
              │           │                    │                      │                    │
              │           └──────┬─────────────┴───────────┬──────────┘                    │
              │                  │                         │                               │
...
:```ascii
               ...
               识别 → 判断 → 产出 三阶段 × 六维度，附每个节点的约束条件

                ┌────────────────────────────────────────────────────────────────────────────┐
                │                        ① 识别模式（是否出现改进信号）                        │
                ├──────────┬──────────┬──────────┬──────────┬──────────┬──────────┬──────────┤
                │  维度    │  skill   │  cmd     │  rule    │  agent   │  hook    │  doc     │
                ├──────────┼──────────┼──────────┼──────────┼──────────┼──────────┼──────────┤
                │  信号    │ 对话暴露 │ 重复 ≥2  │ 用户显式 │ 独立决策 │ git/     │ 路径/    │
                │  识别    │ 技能缺失 │ 固定操作 │ 纠正的行为│ 场景重复  │ claude   │ 命令与   │
                │          │          │          │          │          │ 事件触发  │ 实际不符 │
                ├──────────┼──────────┼──────────┼──────────┼──────────┼──────────┼──────────┤
                │                        ② 对照判断（查已存在文件是否覆盖）                   │
                ├──────────┬──────────┬──────────┬──────────┬──────────┬──────────┬──────────┤
                │  对照    │ plugin/  │ plugin/  │ .claude/ │ .claude/ │ .claude/ │ 项目根   │
                │  对象    │ skills/  │ skills/  │ rules/   │ agents/  │ hooks/   │ 目录文件  │
                │          │ <name>   │ <name>   │ <name>   │ <name>   │ <name>   │          │
                ├──────────┼──────────┼──────────┼──────────┼──────────┼──────────┼──────────┤
                │                        ③ 分类产出（判定条件 → 目标路径）                    │
                ├──────────┬──────────┬──────────┬──────────┬──────────┬──────────┬──────────┤
                │  产出    │ 场景化   │ 确定性    │ 跨项目    │ 有限角色  │ 自动化    │ 补充/   │
                │  类型    │ 解题套路  │ 操作序列   │ 行为约束   │ 持续运行  │ 脚本     │ 修正    │
                │  判定    │ 有分支    │ 步骤固定   │ 通用不涉  │ 需独立    │ 无需交互  │ 仅当前  │
                │          │ 需判断    │ ≤7 步     │ 具体路径   │ 判断力    │ 纯后台   │ 项目有效 │
                └──────────┴──────────┴──────────┴──────────┴──────────┴──────────┴──────────┘
...
| **矩阵图 (Matrix Flow)** | 识别→判断→产出三阶段×六维度的矩阵式流程图，含约束条件备注 | 展示各维度在各阶段的判定逻辑和质量门禁，指导分析和产出决策 |
