---
name: grow-dream
description: 回顾本次对话，分析对 skill / rule / agent / hook / CLAUDE.md / AGENTS.md / memory 是否有改进空间，输出改进建议并可选择直接产出改进文件。识别有规律或频繁提醒的提示并建议写入 memory。之后调用 grill-dream 对改进候选进行追问验收。用户说"改进"、"沉淀"、"提炼规则"、"这个经历能沉淀什么"、"有没有可以提炼成规则的模式"、"造梦"、"总结对话"、"提炼经验"、"固化模式"、"make dream"时触发。
---
...
:```ascii
  ...
  ┌────────────┐   ┌────────────┐   ┌────────────┐   ┌────────────┐   ┌────────────┐   ┌────────────┐   ┌────────────┐   ┌──────────────┐
  │  ① 确定    │   │  ② 浏览    │   │  ③ 项目    │   │  ④ 交叉    │   │  ⑤ 分类    │   │  ⑥ 结构化   │   │  ⑦ 可选    │   │  ⑧ grill-dream│
  │  输入源    │──→│  识别模式   │──→│  文件对照   │──→│  检查矛盾   │──→│  提炼归类   │──→│  输出建议   │──→│  执行产出   │──→│  追问验收     │
  └────────────┘   └────────────┘   └────────────┘   └────────────┘   └────────────┘   └────────────┘   └────────────┘   └──────────────┘
       │                 │                │                 │                │                 │               │               │
       │ 对话记录        │ 重复操作       │ CLAUDE.md       │ 术语一致性     │ skill (场景)    │ 分级建议      │ 按模板写入    │ 高频→固化?  │
       │ 对话日志        │ 显式纠正       │ AGENTS.md       │ 用户纠正跟踪   │ command(确定)   │ 可选方案      │ 格式校验      │ memory→频次?│
       │ git log         │ 隐性模式       │ rules/          │ AI反馈一致性   │ rule (通用)     │               │ 写入 memory   │ skill→成本? │
       │ memory/         │ 频次反馈       │ agents/         │ user 纠正回顾  │ agent/hook/doc  │               │               │ 每候选:判定  │
       └─ 修改历史       └─ 频次统计      └─                └─              └─ memory (规律)  └─              └─              └─
:```
...
9. **grill-dream 追问验收** — 调用 `grill-dream` skill 对改进候选进行交叉拷打（见下节）

## 第⑧步: grill-dream 追问验收

产出改进建议后，**自动调用 `grill-dream` skill（验收模式）** 对每个候选进行追问，防止产出 shelfware（食之无味弃之可惜的固化产物）。

### 验收规则

| 候选类型 | 验收问题 | 通过标准 |
|---------|---------|---------|
| **高频模式** | 实际频次够不够？跨会话通用还是本次特例？ | 频次 ≥2，跨 ≥2 个会话/场景 |
| **memory 候选** | 是否满足"频次≥2 + 跨会话"双门槛？已有条目是否已覆盖？ | 双门槛同时满足 |
| **skill 候选** | 触发条件够明确？与其他 skill 有无重叠？维护成本可接受？ | 独立场景，无已有覆盖 |
| **command 候选** | 步骤是否固定且 ≤7？无分支判断？ | 纯确定性操作 |
| **rule/agent/hook 候选** | 适用面够广？是否过度工程化？ | 通用且必要 |

### 输出

grill-dream 的验收结论**追加**到 grow-dream 输出末尾：
...
- [ ] **grill-dream 验收**：每个改进候选已过 grill-dream 追问，输出明确判定（采纳/放弃/需补充）
