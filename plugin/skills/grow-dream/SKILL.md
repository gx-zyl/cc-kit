...
9. **⑧ 追问验收** — 对第 5 步提炼的每个改进候选，逐一追问验收。这是 grow-dream 的内置子流程，无需外部 skill：
   ...
   > 不问"这个 skill 是否已有文件覆盖"这种可以自己查 `plugin/skills/` 目录的问题。查完文件，带着证据去问。
...
| 产出类型 | 目标路径 | 适用条件 |
|---------|---------|---------|
| **skill** | `plugin/skills/<name>/` 目录（含 `SKILL.md` + `references/` + `scripts/`） | 有判断分支、需要上下文理解的场景化能力。结构参考 `.claude/references/skill-structure.md` |
